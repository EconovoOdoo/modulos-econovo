...
class ZkUserManagement(models.TransientModel):
    ...
    manage_users = fields.Selection(
        [('get_users', 'Get all Users'), ('create_user', 'Create User'), ('update_user', 'Update User'),
         ('delete_user', 'Delete User')],
        'Manage Users', help='User Management', required=True)
    employee_ids = fields.Many2many('hr.employee', string='Employees', compute='_compute_employee_ids')
    employee_id = fields.Many2one(
        'hr.employee', string='Employee', help='Select the Employee', domain="[('id', 'in', employee_ids)]")
    delete_user_selection = fields.Selection(
        [('device_only', 'From Device Only'), ('both_device', 'From Both Device and Odoo')], string='Delete From',
        ...
        for record in self:
            if record.manage_users == 'create_user':
                record.employee_ids = self.env['hr.employee'].search(
                    [('device_id', '!=', int(self.env.context.get('active_id')))]).ids
    ...
    def action_confirm_user_management(self):
        ...
        if self.manage_users:
            if self.manage_users == 'get_users':
                self.env['biometric.device.details'].browse(int(self.env.context.get('active_id'))).get_all_users()
                return {
                    'name': _("ZK Users"),
                ...
                self.env['biometric.device.details'].browse(int(self.env.context.get('active_id'))).set_user(
                    employee_id=self.employee_id.id)
            elif self.manage_users == 'update_user':
                self.env['biometric.device.details'].browse(int(self.env.context.get('active_id'))).update_user(
                    employee_id=self.employee_id.id)
            else:
                self.env['biometric.device.details'].browse(int(self.env.context.get('active_id'))).delete_user(
                    employee_id=self.employee_id.id, employee_user_selection=self.delete_user_selection)
