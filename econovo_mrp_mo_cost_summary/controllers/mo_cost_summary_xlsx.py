...
def _collect_byproduct_entries(data, parent_name, out):
    ...
    Category info is stored in ``data["byproducts_categ_map"]`` (a top-level
    sibling, not inside ``byproducts``) keyed by product_id.  It is not
    injected into the detail dicts either — MoOverviewLine has a strict OWL
    prop shape that rejects unknown keys.
    ...
    categ_map = data.get("byproducts_categ_map") or {}
