...
| 48 | Contact of the current location | `stock.lot.last_delivery_partner_id` only considers **outgoing** transfers (`_get_outgoing_domain`, `stock_lot.py:245`), so it stays empty when a machine is sent to a dealer through an internal transfer. The line field therefore looks at internal transfers too |
| 49 | **The contact was showing the supplier** | Receipts and the chained COMEX transfers inherit the supplier as `partner_id`, so "the last validated transfer" returned the supplier for goods still in the circuit. The criteria now exclude inbound logistics: `picking_type_id.code != 'incoming'` **and** `picking_type_id.is_comex_import = False` (verified `True` on all 8 COMEX picking types in staging). Goods still in the circuit or in own stock have **no** contact |
